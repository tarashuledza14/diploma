...
function buildWorkOrderHtml(
  ...
  return wrap(tr("orders.pdf.documents.workOrder.title", "Work Order"), body);
}
...
function buildEstimateHtml(
  ...
  const priceLabel = esc(
    tr("orders.pdf.documents.common.fields.price", "Price"),
  );
  const totalLabel = esc(
    tr("orders.pdf.documents.common.fields.total", "Total"),
  );
  const unitPriceLabel = esc(
    tr("orders.pdf.documents.common.fields.unitPrice", "Unit price"),
  );
