...
async function main() {
	...
	const partCategories = await Promise.all(
		partCategoryNames.map(name => prisma.partCategory.create({ data: { name } })),
	);
	...
	const manufacturers = await Promise.all(
		entityNames.map(name => prisma.partsManufacturer.create({ data: { name } })),
	);
	...
	const brandByName = Object.fromEntries(brands.map(item => [item.name, item]));
	...
	const vehicleByPlate = Object.fromEntries(
		createdVehicles.map(vehicle => [vehicle.plateNumber ?? vehicle.vin, vehicle]),
	);
	...
	const partSeeds: SeedPart[] = [
		{
			...
			priceRules: [
				...
				{ clientType: ClientType.WHOLESALE, markupPercent: 25, fixedPrice: 156.25 },
			],
			stockMovements: [
				{ type: MovementType.RECEIVED, quantity: 15, reason: 'Початкове оприбуткування', createdAt: daysAgo(145) },
				{ type: MovementType.RECEIVED, quantity: 10, reason: 'Початкове оприбуткування', createdAt: daysAgo(78) },
			],
		},
		{
			key: 'brake-pads',
			...
			inventory: [
				{ quantity: 4, purchasePrice: 1200, location: 'Полиця B-3', receivedAt: daysAgo(50) },
				{ quantity: 5, purchasePrice: 1220, location: 'Полиця B-4', receivedAt: daysAgo(18) },
			],
			priceRules: [{ clientType: ClientType.RETAIL, fixedPrice: 1800 }],
			stockMovements: [
				{ type: MovementType.RECEIVED, quantity: 4, reason: 'Початкове оприбуткування', createdAt: daysAgo(50) },
				{ type: MovementType.RECEIVED, quantity: 5, reason: 'Початкове оприбуткування', createdAt: daysAgo(18) },
			],
		},
		{
			key: 'engine-oil',
			...
			inventory: [
				{ quantity: 24, purchasePrice: 850, location: 'Склад ПММ', receivedAt: daysAgo(33) },
			],
			priceRules: [
			...
			stockMovements: [
				{ type: MovementType.RECEIVED, quantity: 24, reason: 'Початкове оприбуткування', createdAt: daysAgo(33) },
			],
		},
		{
			key: 'spark-plugs',
			...
			inventory: [
				{ quantity: 40, purchasePrice: 80, location: 'Сектор Е', receivedAt: daysAgo(61) },
			],
			priceRules: [{ clientType: ClientType.RETAIL, fixedPrice: 150 }],
			stockMovements: [
				{ type: MovementType.RECEIVED, quantity: 40, reason: 'Початкове оприбуткування', createdAt: daysAgo(61) },
			],
		},
		{
			key: 'cabin-filter',
			...
			inventory: [
				{ quantity: 4, purchasePrice: 260, location: 'Полиця C-1', receivedAt: daysAgo(19) },
			],
			priceRules: [{ clientType: ClientType.RETAIL, fixedPrice: 390 }],
			stockMovements: [
				{ type: MovementType.RECEIVED, quantity: 4, reason: 'Початкове оприбуткування', createdAt: daysAgo(19) },
			],
		},
		{
			key: 'headlight-bulb',
		...
		},
		{
			...
			inventory: [
				{ quantity: 3, purchasePrice: 2100, location: 'Сектор S-2', receivedAt: daysAgo(24) },
			],
			priceRules: [{ clientType: ClientType.RETAIL, fixedPrice: 3250 }],
			stockMovements: [
				{ type: MovementType.RECEIVED, quantity: 3, reason: 'Початкове оприбуткування', createdAt: daysAgo(24) },
			],
		},
		{
			key: 'fuel-pump',
			...
			inventory: [
				{ quantity: 1, purchasePrice: 4200, location: 'Сектор E-1', receivedAt: daysAgo(12) },
			],
			priceRules: [{ clientType: ClientType.RETAIL, fixedPrice: 5900 }],
			stockMovements: [
				{ type: MovementType.RECEIVED, quantity: 1, reason: 'Початкове оприбуткування', createdAt: daysAgo(12) },
			],
		},
		{
			key: 'thermostat',
			...
			inventory: [
				{ quantity: 7, purchasePrice: 540, location: 'Полиця C-4', receivedAt: daysAgo(45) },
			],
			priceRules: [{ clientType: ClientType.RETAIL, fixedPrice: 840 }],
			stockMovements: [
				{ type: MovementType.RECEIVED, quantity: 7, reason: 'Початкове оприбуткування', createdAt: daysAgo(45) },
			],
		},
		{
			key: 'air-filter',
			...
			inventory: [
				{ quantity: 8, purchasePrice: 180, location: 'Сектор A-3', receivedAt: daysAgo(39) },
			],
			priceRules: [{ clientType: ClientType.RETAIL, fixedPrice: 290 }],
			stockMovements: [
				{ type: MovementType.RECEIVED, quantity: 8, reason: 'Початкове оприбуткування', createdAt: daysAgo(39) },
			],
		},
		{
			key: 'shock-absorber',
			...
			inventory: [
				{ quantity: 2, purchasePrice: 1580, location: 'Сектор S-1', receivedAt: daysAgo(27) },
			],
			priceRules: [{ clientType: ClientType.RETAIL, fixedPrice: 2490 }],
			stockMovements: [
				{ type: MovementType.RECEIVED, quantity: 2, reason: 'Початкове оприбуткування', createdAt: daysAgo(27) },
			],
		},
	];

	const partsByKey: Record<string, Awaited<ReturnType<typeof prisma.part.create>>> = {};
	...
	const servicesSeed: SeedService[] = [
		...
		},
		{
			...
			description: 'Зчитування помилок, перевірка основних систем та рекомендації',
			price: 390,
			estimatedTime: 1,
		...
		},
		{
			...
			description: 'Огляд термостата, патрубків та рівня охолоджувальної рідини',
			price: 520,
			estimatedTime: 1.5,
			category: 'Обслуговування',
	...
	const servicesByKey: Record<string, Awaited<ReturnType<typeof prisma.service.create>>> = {};
	...
	async function createOrder(seed: SeedOrder) {
		...
		const manager = createdStaffUsers.find(user => user.fullName === seed.manager)!;
		const mechanic = seed.mechanic
			? createdStaffUsers.find(user => user.fullName === seed.mechanic) ?? null
			: null;
			...
			await prisma.orderService.create({
				data: {
					...
					mechanicId: item.mechanic
						? createdStaffUsers.find(user => user.fullName === item.mechanic)?.id ??
						  mechanic?.id ??
						  null
						: mechanic?.id ?? null,
	...
	const ordersSeed: SeedOrder[] = [
		{
			...
			endDate: daysAgo(11),
			recommendations: 'Рекомендується оглянути гальмівну систему на наступному ТО.',
			parts: [
	...
	for (const [vehicleId, totals] of vehicleTotals.entries()) {
		...
		const latestOrder = vehicleOrders.sort((a, b) => b.startDate.getTime() - a.startDate.getTime())[0];
	...
	const chatSessionsSeed = [
		{
			...
			messages: [
				{ role: 'USER' as const, content: 'Є зараз масляний фільтр і мастило для Golf?', createdAt: daysAgo(1) },
				{
				...
				{ role: 'USER' as const, content: 'Добре, додай ще повітряний фільтр.', createdAt: hoursAgo(18) },
				{
					role: 'ASSISTANT' as const,
					content: 'Додав у підбірку. Повітряний фільтр Mahle є в наявності, підходить під планове ТО.',
		...
		},
		{
			...
			messages: [
				{ role: 'USER' as const, content: 'Що по замовленню на заміну свічок?', createdAt: daysAgo(3) },
				{
					role: 'ASSISTANT' as const,
					content:
				...
				{ role: 'USER' as const, content: 'Чудово, дякую!', createdAt: daysAgo(3) },
			],
		},
		{
			title: 'Склад та низькі залишки',
			messages: [
				{ role: 'USER' as const, content: 'Які позиції треба дозамовити найближчим часом?', createdAt: hoursAgo(6) },
				{
					role: 'ASSISTANT' as const,
					content:
	...
	console.log('✅ Dictionaries, users, clients, vehicles, parts, services and app settings created');
	console.log('✅ Orders, stock movements, documents, chats, notifications and invites created');
