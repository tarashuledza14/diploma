...
export class ServiceService {
  ...
  async deleteService(id: string, actor: AuthUser) {
    ...
    try {
      const result = await this.db.withOrg(organizationId, (tx) =>
        tx.service.deleteMany({ where: { id, organizationId } }),
      );
  ...
  async deleteBulk(ids: string[], actor: AuthUser) {
    const organizationId = this.assertOrganizationId(actor);
    try {
      await this.db.withOrg(organizationId, (tx) =>
        tx.service.deleteMany({ where: { id: { in: ids }, organizationId } }),
      );
    ...
    try {
      return this.db.withOrg(organizationId, async (tx) => {
        ...
        const [services, total] = await Promise.all([
          ...
          tx.service.count({ where: scopedFilters }),
        ]);

        const pageCount = this.paginationService.getPageCount(total, perPage);
        return { data: services, pageCount, total };
      });
    } catch (error) {
  ...
  async getDictionaries() {
    try {
      ...
      return { serviceCategories, partCategories };
    } catch (error) {
